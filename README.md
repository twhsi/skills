...
[`今日的日計畫`](skills/todays-daily-plan/) 是這個 repo 的正式 Skill。它專門服務 Obsidian 的 Mandala Grid 日計畫檔：當你用口語說「上午羽球」「下午三點林君」「晚上合唱」「補今天日記」時，LLM 會把內容分配到今天的九宮時段，並安全追加到正確的 `<!--section: N.x-->` 區塊。
...
| [`fantastical-calendar`](skills/fantastical-calendar/) | 把 LLM 解析出的會議、時段與提醒送到 macOS Fantastical |
